...
- [ ] Смена RU→EN обновляет меню и профиль
- [ ] AI-диалог продолжается на новом языке с следующего сообщения
- [ ] P2P не затрагивается (язык только UI)
- [ ] Выбор сохраняется после рестарта бота

## Технические заметки

- Обновить Redis FSM/cache lang key
- System prompt persona: «respond in user's language»
- Правила (027) показывать на новом языке
